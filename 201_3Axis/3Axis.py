...
3axis_spec = ["implementation_id", "3Axis", 
		 "type_name",         "3Axis", 
...
# 
# 
class 3Axis(OpenRTM_aist.DataFlowComponentBase):
	...
	def onExecute(self, ec_id):
		...
		self._d_axis.data[0]  = axes['x']
		self._d_axis.data[1]  = axes['y']
		self._d_axis.data[2]  = axes['z']
		
		print ("x : {xv}, y : {yv}, z : {zv}").format(xv=self._d_axis[0], yv=self._d_axis[1], zv=self._d_axis[2])
...
def 3AxisInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=3axis_spec)
...
def MyModuleInit(manager):
    ...
    # Create a component
    comp = manager.createComponent("3Axis")
