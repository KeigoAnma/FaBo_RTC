...
irled_spec = ["implementation_id", "IRLED", 
		 "type_name",         "IRLED", 
...
# 
# 
class IRLED(OpenRTM_aist.DataFlowComponentBase):
	...
	def __init__(self, manager):
		...
		"""
		If RTC get some signal (Boolean) , send IRLED signal to IR receiver.
		"""
	...
	def onActivated(self, ec_id):
		...
		GPIO.setup(self._port_num[0], GPIO.OUT)
	        print "set up OUT PUT"
		return RTC.RTC_OK
	...
	def onExecute(self, ec_id):
		    ...
		    if(self._d_signal.data):
		    	    GPIO.output(self._port_num[0], True)
		    else:
			    GPIO.output(self._port_num[0], False)
                else:
		    GPIO.output(self._port_num[0], False)
                
                print self._d_signal.data
		return RTC.RTC_OK
...
def IRLEDInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str=irled_spec)
...
def MyModuleInit(manager):
    ...
    # Create a component
    comp = manager.createComponent("IRLED")
